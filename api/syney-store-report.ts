...
export default async function handler(
  ...
  try {
    ...
    const storeInNo = parseProxyIdentifier(body.storeInNo)
    ...
    console.error('syney_proxy_failure', safeErrorMetadata(error))
    response.status(502).json({ error: '西尼入库单代理服务暂不可用' })
  }
}
