-- Day 3 草稿：员工隔离 RLS 策略
-- 说明：
-- 1. 这是设计稿级 SQL，不建议未经验证直接作为正式 migration 应用
-- 2. 该草稿默认管理员身份也来源于 public.employees.role = 'admin'
-- 3. 该草稿假定 public.production_orders.employee_id -> public.employees.id
-- 4. 该草稿假定 public.production_order_items.order_id -> public.production_orders.id
...
create policy "Production orders admin all"
...
create policy "Production orders employee select own"
  on public.production_orders
  for select
  to authenticated
  using (employee_id = public.current_employee_id());

create policy "Production orders employee insert own"
  on public.production_orders
  for insert
  to authenticated
  with check (employee_id = public.current_employee_id());

create policy "Production orders employee update own"
  on public.production_orders
  for update
  to authenticated
  using (employee_id = public.current_employee_id())
...
create policy "Production order items admin all"
...
create policy "Production order items employee select own"
  on public.production_order_items
  for select
  to authenticated
  using (
...
create policy "Production order items employee insert own"
  on public.production_order_items
  for insert
  to authenticated
  with check (
...
create policy "Production order items employee update own"
  on public.production_order_items
  for update
  to authenticated
  using (
...
create policy "Production order items employee delete own"
  on public.production_order_items
  for delete
  to authenticated
  using (
...
-- 建议验证点：
-- 1. admin 能查全量 employees / production_orders / production_order_items
-- 2. employee 只能查自己的 employees
-- 3. employee 只能查自己的工单与明细
-- 4. employee 不能把 production_orders.employee_id 写成别人
-- 5. employee 不能改别人订单下的明细