...
create or replace function public.sync_process_standards_cost_fields() returns trigger language plpgsql as $$ begin if exists (
    select 1
    from pg_attribute
...
new.labor_cost := (
  (
    (
      (
        (coalesce(new.standard_seconds, 0))::numeric * coalesce(new.labor_rate, 0)
      ) / 3600::numeric
...
new.total_cost := (
  (
    (
      ...
      (
        (coalesce(new.theoretical_seconds, 0))::numeric * coalesce(new.equipment_rate, 0)
      ) / 3600::numeric
    ) + (
      coalesce(new.tool_rate, 0) + coalesce(new.cutting_fluid_rate, 0) + coalesce(new.fixture_rate, 0)
    ) + (
      (
        (coalesce(new.inspection_seconds, 0))::numeric * coalesce(new.labor_rate, 0)
      ) / 3600::numeric
    ) + case
...
create trigger sync_process_standards_cost_fields before
insert
  or
update on public.process_standards for each row execute function public.sync_process_standards_cost_fields();
