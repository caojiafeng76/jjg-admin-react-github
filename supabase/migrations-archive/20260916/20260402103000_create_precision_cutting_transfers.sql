...
create index if not exists idx_precision_cutting_transfers_project_no on public.precision_cutting_transfers (project_no);
create index if not exists idx_precision_cutting_transfers_target_workshop on public.precision_cutting_transfers (target_workshop);
create index if not exists idx_precision_cutting_transfers_created_at_desc on public.precision_cutting_transfers (created_at desc);
create index if not exists idx_precision_cutting_transfers_operator_names_gin on public.precision_cutting_transfers using gin (operator_names);
create or replace function public.sync_precision_cutting_transfer_audit_fields() returns trigger language plpgsql as $$ begin if new.is_audited is true then if tg_op = 'INSERT' then new.audited_at = coalesce(new.audited_at, now());
...
$$;
drop trigger if exists precision_cutting_transfers_set_updated_at on public.precision_cutting_transfers;
create trigger precision_cutting_transfers_set_updated_at before
update on public.precision_cutting_transfers for each row execute function public.update_updated_at_column();
drop trigger if exists sync_precision_cutting_transfer_audit_fields on public.precision_cutting_transfers;
create trigger sync_precision_cutting_transfer_audit_fields before
insert
  or
update of is_audited,
  audited_at on public.precision_cutting_transfers for each row execute function public.sync_precision_cutting_transfer_audit_fields();
alter table public.precision_cutting_transfers enable row level security;
drop policy if exists "Precision cutting transfers admin all" on public.precision_cutting_transfers;
create policy "Precision cutting transfers admin all" on public.precision_cutting_transfers for all to authenticated using (public.is_admin()) with check (public.is_admin());