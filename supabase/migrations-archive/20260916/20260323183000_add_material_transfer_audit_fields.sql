...
create or replace function public.sync_material_transfer_audit_fields() returns trigger language plpgsql as $$ begin if new.is_audited is true then if tg_op = 'INSERT' then new.audited_at = coalesce(new.audited_at, now());
...
update of is_audited,
  audited_at on public.material_transfers for each row execute function public.sync_material_transfer_audit_fields();