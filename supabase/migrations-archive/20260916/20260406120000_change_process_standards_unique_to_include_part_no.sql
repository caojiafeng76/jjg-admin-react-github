...
alter table public.production_order_items drop constraint if exists production_order_items_product_model_operation_fkey;
-- 2. 删除旧的 (model, operation) 唯一约束
alter table public.process_standards drop constraint if exists process_standards_model_operation_key;
...
add constraint process_standards_model_operation_part_no_key unique nulls not distinct (model, operation, part_no);