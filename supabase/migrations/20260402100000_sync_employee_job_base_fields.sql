...
begin
  if exists (
    ...
    alter table public.employees rename column performance to coefficient;
  end if;
end $$;
...
begin
  if new.job_name is null then
    new.hourly_wage = round(coalesce(new.hourly_wage, 0)::numeric, 2);
    return new;
  end if;
...
insert
  or
update of job_name, hourly_wage on public.employees for each row execute function public.apply_job_base_hourly_fee_to_employee();
...
begin
  target_hourly_fee = round(coalesce(new.hourly_fee, 0)::numeric, 2);
  ...
  where (
    ...
    and e.hourly_wage is distinct from target_hourly_fee;
...
where e.job_name = jbs.job_name
  and e.hourly_wage is distinct from round(coalesce(jbs.hourly_fee, 0)::numeric, 2);