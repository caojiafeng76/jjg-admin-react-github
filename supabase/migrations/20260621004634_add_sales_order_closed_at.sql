...
create or replace function public.set_sales_order_closed_at() returns trigger
language plpgsql
as $$
begin
	if new.status = '已结案' then
...
create trigger set_sales_order_closed_at
before insert or update of status on public.sales_orders
for each row
execute function public.set_sales_order_closed_at();
