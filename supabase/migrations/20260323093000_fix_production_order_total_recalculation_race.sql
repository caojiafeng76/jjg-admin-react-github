create or replace function public.recalculate_production_order_totals(
...
begin
  ...
  set
    total_qualified_hours = totals.total_qualified_hours,
    efficiency = case
  ...
  from (
    select round(
      coalesce(
        sum(
          coalesce(poi.qualified_hours, 0)
          + coalesce(poi.bonus_seconds, 0) / 3600.0
          - coalesce(poi.defect_hours, 0)
        ),
        0
      )::numeric,
      2
...
create or replace function public.update_production_order_totals()
returns trigger
language plpgsql
as $$
begin
  if tg_op = 'UPDATE' and new.order_id is distinct from old.order_id then
    perform public.recalculate_production_order_totals(old.order_id);
  end if;
...
update public.production_orders as po
set
  total_qualified_hours = order_totals.total_qualified_hours,
  efficiency = case
