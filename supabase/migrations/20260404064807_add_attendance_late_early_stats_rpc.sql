...
as $$
with
-- 每人每天打卡：取最早/最晚两次时间，并关联生产工单获取班别和工时
base as (
  select
    ad.name,
    ad.date,
...
-- 夜班早退辅助：取次日清晨(< 12:00)的第一次打卡视为夜班下班时间
next_morning as (
...
early_leave_records as (
  ...
  where b.work_hours is not null
    and (
      ...
      or
      -- 夜班跨午夜(工时>5h): 次日清晨打卡 < (19:00 + 工时) mod 86400 - 5min
      (
      ...
      or
      -- 夜班不跨午夜(工时<=5h): 当日最后打卡 < 19:00 + 工时 - 5min
      (
        b.shift = '夜班' and b.work_hours <= 5
