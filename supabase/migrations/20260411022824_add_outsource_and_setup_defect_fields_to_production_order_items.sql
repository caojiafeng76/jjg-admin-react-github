...
alter table public.production_order_items
  drop column if exists defect_hours;

alter table public.production_order_items
  add column defect_hours numeric generated always as (
    (
      case
        when defect_reason_1 = '加工' then ((defect_quantity_1 * 2)::numeric * standard_seconds)
        else 0::numeric
      end +
      case
        when defect_reason_2 = '加工' then ((defect_quantity_2 * 2)::numeric * standard_seconds)
        else 0::numeric
      end
    ) / 3600.0
  ) stored;
...
create or replace function public.recalculate_production_order_totals(target_order_id uuid)
returns void
language plpgsql
as $$
begin
  ...
  from (
    select round(
      (
        coalesce(
          sum(coalesce(poi.qualified_hours, 0) - coalesce(poi.defect_hours, 0)),
          0
