create table if not exists public.machine_equipment_maintenances (
  ...
  depreciation_rate numeric(14, 8) generated always as (
    case
      when depreciation_years > 0 and annual_runtime_hours > 0::numeric then (
        machine_value /
  ...
  constraint machine_equipment_maintenances_unified_device_no_not_blank check (
    btrim(unified_device_no) <> ''
  ),
  constraint machine_equipment_maintenances_operation_not_blank check (
    btrim(operation) <> ''
  ),
  constraint machine_equipment_maintenances_machine_name_not_blank check (
    btrim(machine_name) <> ''
  ),
  constraint machine_equipment_maintenances_original_no_not_blank check (
    original_no is null or btrim(original_no) <> ''
  ),
  constraint machine_equipment_maintenances_power_kw_non_negative check (
    power_kw >= 0::numeric
  ),
  constraint machine_equipment_maintenances_sync_work_quantity_positive check (
    sync_work_quantity > 0
  ),
  constraint machine_equipment_maintenances_electricity_unit_price_non_negative check (
    electricity_unit_price >= 0::numeric
  ),
  constraint machine_equipment_maintenances_machine_value_non_negative check (
    machine_value >= 0::numeric
  ),
  constraint machine_equipment_maintenances_depreciation_years_positive check (
    depreciation_years > 0
  ),
  constraint machine_equipment_maintenances_annual_runtime_hours_positive check (
    annual_runtime_hours > 0::numeric
  )
);
...
create unique index if not exists idx_machine_equipment_maintenances_unified_device_no_unique
  on public.machine_equipment_maintenances (unified_device_no);
create index if not exists idx_machine_equipment_maintenances_updated_at_desc
  on public.machine_equipment_maintenances (updated_at desc);
create index if not exists idx_machine_equipment_maintenances_operation
  on public.machine_equipment_maintenances (operation);
create index if not exists idx_machine_equipment_maintenances_machine_name
  on public.machine_equipment_maintenances (machine_name);

drop trigger if exists update_machine_equipment_maintenances_updated_at on public.machine_equipment_maintenances;
create trigger update_machine_equipment_maintenances_updated_at
before update on public.machine_equipment_maintenances
...
create policy "Machine equipment maintenances admin all"
on public.machine_equipment_maintenances
for all
to authenticated
