...
generated always as (
  (((standard_seconds)::numeric * labor_rate) / 3600::numeric)::numeric(12, 4)
) stored,
add column if not exists equipment_cost numeric(12, 4)
generated always as (
  (((theoretical_seconds)::numeric * equipment_rate) / 3600::numeric)::numeric(12, 4)
) stored,
add column if not exists tooling_consumable_cost numeric(12, 4)
generated always as (
  (tool_rate + cutting_fluid_rate + fixture_rate)::numeric(12, 4)
) stored,
add column if not exists inspection_cost numeric(12, 4)
generated always as (
  (((inspection_seconds)::numeric * labor_rate) / 3600::numeric)::numeric(12, 4)
) stored,
add column if not exists overhead_cost numeric(12, 4)
generated always as (
  case
    when daily_total_hours > 0::numeric then (
      ((daily_management_cost * (standard_seconds)::numeric)
      / (3600::numeric * daily_total_hours))::numeric(12, 4)
    )
    else 0::numeric(12, 4)
  end
) stored,
