...
alter table public.material_transfers
drop constraint if exists material_transfers_operator_employee_ids_not_empty,
  drop constraint if exists material_transfers_operator_arrays_same_length,
  add constraint material_transfers_operator_employee_ids_not_empty check (coalesce(array_length(operator_employee_ids, 1), 0) > 0),
