...
create index if not exists idx_process_standards_equipment_no
  on public.process_standards (equipment_no);
...
do $$ begin
  if not exists (
    ...
    alter table public.process_standards
    add constraint process_standards_equipment_no_fkey
...
create or replace function public.apply_machine_equipment_hourly_rate_to_process_standard()
returns trigger
language plpgsql
as $$
declare
...
create trigger apply_machine_equipment_hourly_rate_to_process_standard
before insert or update of equipment_no on public.process_standards
for each row
execute function public.apply_machine_equipment_hourly_rate_to_process_standard();
...
begin
  ...
  if tg_op = 'UPDATE'
    ...
    and new_hourly_rate is not distinct from old_hourly_rate then
    return new;
  end if;
  ...
  where (
    ...
    and ps.equipment_rate is not distinct from old_hourly_rate;
...
for each row
execute function public.sync_process_standard_equipment_rate_from_machine_equipment_maintenance();