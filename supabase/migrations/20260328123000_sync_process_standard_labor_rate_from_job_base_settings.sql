create or replace function public.apply_job_base_hourly_fee_to_process_standard()
returns trigger
language plpgsql
as $$
...
begin
  ...
  if tg_op = 'UPDATE'
    and new.job_name is not distinct from old.job_name then
    return new;
  end if;

  select jbs.hourly_fee
  into matched_hourly_fee
  from public.job_base_settings as jbs
  where jbs.job_name = new.job_name
  limit 1;

  if matched_hourly_fee is null then
    return new;
  end if;
  ...
  if (
    ...
    tg_op = 'UPDATE'
    and new.job_name is distinct from old.job_name
    and (
      new.labor_rate is null
      or coalesce(new.labor_rate, 0) = 0
      or new.labor_rate is not distinct from old.labor_rate
    )
  ) then
    new.labor_rate = round(matched_hourly_fee::numeric, 4);
  end if;
...
create or replace function public.sync_process_standard_labor_rate_from_job_base_setting()
returns trigger
language plpgsql
as $$
...
begin
  if tg_op = 'UPDATE' then
    old_hourly_fee = round(coalesce(old.hourly_fee, 0)::numeric, 4);
  else
    old_hourly_fee = round(coalesce(new.hourly_fee, 0)::numeric, 4);
  end if;
  ...
  if tg_op = 'UPDATE'
    ...
    and new_hourly_fee is not distinct from old_hourly_fee then
    return new;
  end if;
  ...
  where (
    ...
    and ps.labor_rate is not distinct from old_hourly_fee;
...
create trigger sync_process_standard_labor_rate_from_job_base_setting after
insert
  or
...
where ps.job_name = jbs.job_name
  and ps.labor_rate is distinct from round(jbs.hourly_fee::numeric, 4);
