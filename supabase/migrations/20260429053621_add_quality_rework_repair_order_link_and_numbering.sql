...
alter table public.quality_rework_repairs
  drop constraint if exists quality_rework_repairs_project_no_not_blank;
...
create index if not exists idx_quality_rework_repairs_project_no
on public.quality_rework_repairs (project_no)
...
select to_date(substring(document_no from 1 for 8), 'YYYYMMDD') as document_date,
  max(substring(document_no from 9)::integer) as last_sequence
from public.quality_rework_repairs
where document_no ~ '^\d{11}$'
group by to_date(substring(document_no from 1 for 8), 'YYYYMMDD')
on conflict (document_date) do update
...
create or replace function public.next_quality_rework_repair_document_no(
...
as $$
declare
  v_document_date date := coalesce(p_document_date, timezone('Asia/Shanghai', now())::date);
  v_sequence integer;
begin
  if coalesce(auth.role(), '') in ('anon', 'authenticated')
    and not public.current_user_has_permission('page:quality-rework-repair') then
    raise exception '没有返工返修权限，无法生成编号' using errcode = '42501';
  end if;

  insert into public.quality_rework_repair_document_sequences (
    document_date,
    last_sequence
  )
  values (v_document_date, 1)
  on conflict (document_date) do update
...
revoke all on function public.next_quality_rework_repair_document_no(date) from public;
grant execute on function public.next_quality_rework_repair_document_no(date) to authenticated;

create or replace function public.assign_quality_rework_repair_document_no()
returns trigger
language plpgsql
security definer
set search_path = public
as $$
begin
  if nullif(btrim(new.document_no), '') is null then
    new.document_no := public.next_quality_rework_repair_document_no();
  else
...
create trigger assign_quality_rework_repair_document_no
before insert on public.quality_rework_repairs
for each row execute function public.assign_quality_rework_repair_document_no();;
