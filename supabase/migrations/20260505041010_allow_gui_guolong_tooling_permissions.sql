...
do $$
begin
  if to_regclass('public.tooling_inventory') is not null
    and not exists (
      select 1
      from pg_policies
  ...
  then
    execute 'create policy "tooling_inventory permission rw" on public.tooling_inventory for all to authenticated using (public.current_user_has_permission(''page:tooling-inventory'')) with check (public.current_user_has_permission(''page:tooling-inventory''))';
  end if;
  ...
  then
    execute 'create policy "tooling_stock_in permission rw" on public.tooling_stock_in for all to authenticated using (public.current_user_has_permission(''page:tooling-stock-in'')) with check (public.current_user_has_permission(''page:tooling-stock-in''))';
  end if;
  ...
  then
    execute 'create policy "tooling_stock_out permission rw" on public.tooling_stock_out for all to authenticated using (public.current_user_has_permission(''page:tooling-stock-out'')) with check (public.current_user_has_permission(''page:tooling-stock-out''))';
  end if;
end $$;
...
declare
  v_table oid;
begin
  v_table := to_regclass('public.tooling_inventory');
  ...
  then
    execute 'create trigger prevent_viewer_dml_trigger before insert or update or delete on public.tooling_inventory for each row execute function public.prevent_viewer_dml()';
  end if;
  ...
  then
    execute 'create trigger prevent_viewer_dml_trigger before insert or update or delete on public.tooling_stock_in for each row execute function public.prevent_viewer_dml()';
  end if;
  ...
  then
    execute 'create trigger prevent_viewer_dml_trigger before insert or update or delete on public.tooling_stock_out for each row execute function public.prevent_viewer_dml()';
  end if;
end $$;;
