-- 允许拥有“别墅梯订单-贴膜完成”权限的查看员仅标记 film_date
-- 其他 viewer 写操作仍由 prevent_viewer_dml 阻止

create or replace function public.prevent_viewer_dml() returns trigger
language plpgsql
security definer
...
begin
  if public.current_user_is_viewer() then
    if tg_table_schema = 'public'
      ...
      and tg_op = 'UPDATE'
      and public.current_user_has_permission('feature:villa-lift-order.mark-film')
...
where e.name = '吴雯雯'
  and p.key = 'feature:villa-lift-order.mark-film'
