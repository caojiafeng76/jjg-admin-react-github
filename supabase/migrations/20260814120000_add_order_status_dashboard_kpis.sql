-- 订单现状看板：为 get_order_status_dashboard_v2 增加全局 KPI 聚合输出（kpis）
-- 在 filtered_orders（完整筛选集，非分页切片）上聚合：
--   状态分布（正常/预警/延期）、平均完工率、平均成品率、生产明细数、物料转移数
...
set search_path = public as $$ with normalized_params as (
  ...
  kpi_summary as (
    select count(*) filter (
        where production_status = '正常'
      )::integer as normal_order_count,
      count(*) filter (
        where production_status = '预警'
      )::integer as warning_order_count,
      count(*) filter (
        where production_status = '延期'
      )::integer as overdue_order_count,
      round(coalesce(avg(completion_rate), 0)::numeric, 1) as avg_completion_rate,
      round(coalesce(avg(yield_rate), 0)::numeric, 1) as avg_yield_rate,
      coalesce(sum(production_item_count), 0)::integer as production_item_count,
      coalesce(sum(transfer_record_count), 0)::integer as material_transfer_count
    from filtered_orders
  ),
  paged_orders as (
  ...
  extrusion_summary as (
    select epi.project_no,
      coalesce(sum(epi.theoretical_output_count), 0)::numeric as extrusion_quantity
    from public.extrusion_production_items as epi
      join public.extrusion_productions as ep on ep.id = epi.extrusion_production_id
    where nullif(btrim(epi.project_no), '') is not null
      and ep.is_audited = true
    group by epi.project_no
  ),
  extrusion_details as (
    select epi.project_no,
      jsonb_agg(
        jsonb_build_object(
          'createdAt',
          ...
          ep.shift,
          'shiftLeaderName',
          ep.shift_leader_name,
          'isAudited',
          ep.is_audited,
          'actualOutputLengthMm',
          epi.actual_output_length_mm,
          'orderLengthMm',
          epi.order_length_mm,
          'actualQuantity',
          epi.actual_quantity,
          'theoreticalOutputCount',
          epi.theoretical_output_count,
          'actualOutputWeightKg',
          epi.actual_output_weight_kg,
          'materialYield',
          epi.material_yield
        )
    ...
    from public.extrusion_production_items as epi
      join public.extrusion_productions as ep on ep.id = epi.extrusion_production_id
      join paged_orders as orders on orders.project_no = epi.project_no
    group by epi.project_no
  ),
  ...
  items as (
    select orders.created_at,
      ...
      jsonb_build_object(
        ...
        'extrusionQuantity',
        coalesce(ext_summary.extrusion_quantity, 0),
        'extrusionDetails',
        coalesce(ext_details.rows, '[]'::jsonb),
        'transferQuantity',
    ...
    from paged_orders as orders
      ...
      left join extrusion_summary as ext_summary on ext_summary.project_no = orders.project_no
      left join extrusion_details as ext_details on ext_details.project_no = orders.project_no
  )
select jsonb_build_object(
    ...
    (
      select jsonb_build_object(
          ...
          'normalOrderCount',
          kpi_summary.normal_order_count,
          'warningOrderCount',
          kpi_summary.warning_order_count,
          'overdueOrderCount',
          kpi_summary.overdue_order_count,
          'avgCompletionRate',
          kpi_summary.avg_completion_rate,
          'avgYieldRate',
          kpi_summary.avg_yield_rate,
          'productionItemCount',
          kpi_summary.production_item_count,
          'materialTransferCount',
          kpi_summary.material_transfer_count
        )
      from kpi_summary
    )
  );
