create table if not exists public.notifications (
  ...
  constraint notifications_action_type_check check (action_type in ('create', 'update'))
);
...
create or replace function public.sync_notification_read_at() returns trigger language plpgsql as $$ begin if new.is_read is true then if tg_op = 'INSERT' then new.read_at = coalesce(new.read_at, now());
elsif old.is_read is distinct
from new.is_read then new.read_at = coalesce(new.read_at, now());
end if;
...
create trigger sync_notification_read_at before
insert
  or
update of is_read,
  read_at on public.notifications for each row execute function public.sync_notification_read_at();
create or replace function public.create_notification_for_production_order() returns trigger language plpgsql as $$
...
insert
  or
update on public.production_orders for each row execute function public.create_notification_for_production_order();
create or replace function public.create_notification_for_material_transfer() returns trigger language plpgsql as $$
declare actor_id uuid;
actor_display_name text;
begin actor_id := public.current_employee_id();
if actor_id is null then return new;
...
insert
  or
update on public.material_transfers for each row execute function public.create_notification_for_material_transfer();
alter table public.notifications enable row level security;
drop policy if exists "Notifications insert own" on public.notifications;
create policy "Notifications insert own" on public.notifications for
insert to authenticated with check (actor_employee_id = public.current_employee_id());
drop policy if exists "Notifications admin select all" on public.notifications;
create policy "Notifications admin select all" on public.notifications for
...
create policy "Notifications admin update all" on public.notifications for
