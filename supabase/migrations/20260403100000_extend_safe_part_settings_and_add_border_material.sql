...
COMMENT ON COLUMN syney_safe_part_settings.decomposition_role IS
  '分解单列映射角色。取值：side_frame（侧围）、cross_frame（横围）、'
  'front_plate（前板）、upper_middle（上中板）、lower_middle（下中板）、'
  'rear_upper（上后板）、rear_lower（下后板）、'
  'extension_upper（上加长板）、extension_lower（下加长板）';
...
COMMENT ON COLUMN "syney-pos"."BorderMaterial" IS
  '围框垫材质。取值：橡胶（默认）、尼龙';
