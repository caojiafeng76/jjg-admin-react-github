...
create index if not exists idx_process_standards_job_name
on public.process_standards (job_name);
...
begin
  if not exists (
    ...
    alter table public.process_standards
    add constraint process_standards_job_name_fkey
    foreign key (job_name)
    references public.job_base_settings (job_name)
    on update cascade
    on delete restrict;
  end if;
end $$;
...
with mapped_jobs as (
  select
    ...
    case
      ...
      when operation like '冲%' or operation like '%打孔%' then '冲床'
      else null
    end as job_name
