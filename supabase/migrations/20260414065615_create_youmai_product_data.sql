...
create unique index if not exists idx_youmai_product_data_material_code_unique
on public.youmai_product_data (material_code);

create index if not exists idx_youmai_product_data_updated_at_desc
on public.youmai_product_data (updated_at desc);

drop trigger if exists update_youmai_product_data_updated_at on public.youmai_product_data;
create trigger update_youmai_product_data_updated_at
before update on public.youmai_product_data
...
create policy "Youmai product data admin all"
