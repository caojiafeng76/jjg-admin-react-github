...
serve(async (request) => {
  ...
  if (!authorization) {
    return jsonResponse({ error: '未登录，无法解绑账号' }, 401)
  }
  ...
  if (!employee.auth_user_id) {
    return jsonResponse({ error: '该员工当前没有绑定账号' }, 409)
  }

  const { error: unbindError } = await adminClient
    .from('employees')
    .update({ auth_user_id: null })
    .eq('id', employee.id)

  if (unbindError) {
    return jsonResponse({ error: '解绑员工账号失败' }, 500)
  }
