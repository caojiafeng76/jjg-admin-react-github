...
export interface ProductionOrderExportEmployee {
  id?: string | null
  name?: string | null
...
export interface ProductionOrderExportItem {
  id: string
...
export interface ProductionOrderForExportRecord {
  id: string
  created_at: string
  order_date: string
  ...
  shift?: string | null
  remark?: string | null
  employee?: ProductionOrderExportEmployee | null
  items: ProductionOrderExportItem[]
}
...
function setRowHeight(ws: XLSX.WorkSheet, rowHeight: number, rowCount: number) {
...
function centerAllCells(ws: XLSX.WorkSheet, data: Array<Array<unknown>>) {
  if (!data || data.length === 0) return
...
function getEmployeeJobName(orders: ProductionOrderForExportRecord[]) {
  return (
    orders.find((order) => order.employee?.job_name)?.employee?.job_name || ''
  )
}

function getEmployeeHourlyWage(orders: ProductionOrderForExportRecord[]) {
  return roundToTwo(
    normalizeNumber(
...
function getEmployeeCoefficient(orders: ProductionOrderForExportRecord[]) {
...
function compareEmployeeGroupsByJobPresence(
  [, leftOrders]: [string, ProductionOrderForExportRecord[]],
  [, rightOrders]: [string, ProductionOrderForExportRecord[]],
) {
...
function getOrderPositiveQualifiedHours(order: ProductionOrderForExportRecord) {
  return Number(
    order.items
...
function buildExportRow(
  order: ProductionOrderForExportRecord,
  item: ProductionOrderExportItem,
...
function buildEmptyExportRow(order: ProductionOrderForExportRecord): ExportRow {
...
function buildSummarySheetRows(
  employeeGroups: Array<[string, ProductionOrderForExportRecord[]]>,
) {
...
function createProductionOrderWorkbook(
  orders: ProductionOrderForExportRecord[],
) {
  const workbook = XLSX.utils.book_new()
  const employeeGroups = new Map<string, ProductionOrderForExportRecord[]>()
