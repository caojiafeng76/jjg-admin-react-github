...
serve(async (request) => {
  ...
  if (!authorization) {
    return jsonResponse({ error: '未登录，无法重置密码' }, 401)
  }
  ...
  let payload: { employeeId?: string; password?: string }
  ...
  if (!password || password.length < 6) {
    return jsonResponse({ error: '新密码至少需要 6 位' }, 400)
  }
  ...
  if (updateError) {
    return jsonResponse({ error: updateError.message || '重置密码失败' }, 400)
  }
