...
async function getProductionDailyReportIdsPageByFilters(
  adminClient: Awaited<ReturnType<typeof assertAdmin>>['adminClient'],
  {
    page,
    pageSize,
  ...
  const { data, error, count } = await applyProductionDailyReportOrdering(
    buildProductionDailyReportQuery(
      adminClient,
      filters,
      'id, production_orders!inner(order_date)',
      true,
    ),
  ).range(from, to)

  if (error) {
    throw new Error(`获取导出日报 ID 失败: ${error.message}`)
  }
...
async function getProductionDailyReportIdsByFilters(
  adminClient: Awaited<ReturnType<typeof assertAdmin>>['adminClient'],
  filters: ProductionDailyReportFilters,
) {
  const firstPage = await getProductionDailyReportIdsPageByFilters(
    adminClient,
    {
      page: 1,
  ...
  for (
    ...
    const pageResults = await Promise.all(
      pageChunk.map((page) =>
        getProductionDailyReportIdsPageByFilters(adminClient, {
          page,
          pageSize: EXPORT_PAGE_SIZE,
          filters,
        }),
