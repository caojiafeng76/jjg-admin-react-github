...
- [x] **C1** 首页 Dashboard：欢迎空卡 → 决策优先看板（4-6 张 KPI 卡，大数字 + 环比 + 趋势迷你图）
- [x] **C2** 登录 Login：硬编码 `blue-600/700` → 品牌 token，与主站统一
- [x] **C3** 404 / 403 / 错误页：统一卡片圆角与空态插画风格
...
- [x] **F3** 生产日报表 ProductionDailyReport（汇总行突出 + 数字列 tabular-nums）
...
- [x] **S1** 表格三态通用化：骨架屏加载 / 引导式空态 / 错误重试（`src/ui/TableState.tsx`，刀具资料页示范接入）
- [x] **S2** 状态徽章体系化（成功=绿 / 警告=琥珀 / 危险=红 / 中性=灰 pill，`src/ui/StatusPill.tsx`，车间订单状态列示范接入）
- [x] **S3** 表单反馈文案："失败原因 + 影响 + 建议"（`src/ui/FormErrorAlert.tsx`，刀具资料表单示范接入）
- [x] **S4** 选中摘要条提炼为通用组件（`src/ui/SelectedSummaryBar.tsx`，F1 落地，F3/SpecList/SafePartSetting 已复用）
