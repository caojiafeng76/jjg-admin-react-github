...
- 本次不实现审批流、打印/PDF 导出、附件上传或自动编号规则。
...
- **编号字段使用可选 `document_no`。** 原表有 `NO`，但未给出自动编号规则；先支持人工录入并建立唯一索引，避免提前固化错误规则。
...
- [Risk] 原 Word 表单没有完整自动编号规则，人工编号可能出现空值或重复输入。 → 允许为空，非空编号唯一；后续明确规则后再增加自动生成。
