...
const CREATE_PERMISSION_KEY = 'feature:packaging-process-employee-list.create'
const EDIT_PERMISSION_KEY = 'feature:packaging-process-employee-list.edit'
const DELETE_PERMISSION_KEY = 'feature:packaging-process-employee-list.delete'

export default function PackagingEmployeeListPage() {
  ...
  const [modalTitle, setModalTitle] = useState('新建员工')
  const [isEdit, setIsEdit] = useState(false)
  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([])
  const [editingRecord, setEditingRecord] = useState<PackagingEmployee | null>(
    null,
  )
  const [formRef, setFormRef] =
    useState<FormInstance<PackagingEmployeeFormValues> | null>(null)
  ...
  const { data, isLoading } = usePackagingEmployeeList({
    page,
    pageSize,
    searchParams,
  })

  const createMutation = useCreatePackagingEmployee()
  const updateMutation = useUpdatePackagingEmployee()
  const deleteMutation = useDeletePackagingEmployee()
  ...
  const handleFinish = useCallback(
    async (values: PackagingEmployeeFormValues) => {
      if (viewerDenied) {
