...
interface Props {
  onSearch: (values: PrecisionCuttingTransferSearchValues) => void
  onReset: () => void
  initialValues?: PrecisionCuttingTransferSearchValues
  mobile?: boolean
}
...
export default function MaterialTransferSearch({
  ...
  function handleFinish(values: PrecisionCuttingTransferSearchValues) {
    setIsSearching(true)
    onSearch({
      startDate:
  ...
  const formContent = (
    ...
    >
      ...
      <Form.Item name="operatorName" className={formItemClassName}>
        <Select
          placeholder="操作人"
          allowClear
      ...
      <Form.Item name="recipientName" className={formItemClassName}>
        <Input
          placeholder="接收人"
          allowClear
          onPressEnter={() => form.submit()}
      ...
      <Form.Item name="isAudited" className={formItemClassName}>
        <Select
          ...
          options={[...PRECISION_CUTTING_TRANSFER_AUDIT_OPTIONS]}
          className="rounded-lg"
