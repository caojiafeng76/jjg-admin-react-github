...
import JobBaseSettingForm from './JobBaseSettingForm'
import JobBaseSettingSearch from './JobBaseSettingSearch'
import JobBaseSettingTable from './JobBaseSettingTable'
import {
  useCreateJobBaseSetting,
  useDeleteJobBaseSettings,
  useJobBaseSettingsList,
  useUpdateJobBaseSetting,
} from './useJobBaseSettings'

export default function JobBaseSettingPage() {
  const { message } = App.useApp()

  const [isModalOpen, setIsModalOpen] = useState(false)
  const [modalTitle, setModalTitle] = useState('新建岗位基础数值')
  ...
  const [searchParams, setSearchParams] = useState<{
    ...
    jobName: searchParamsURL.get('jobName') || undefined,
  })

  const { data, isLoading } = useJobBaseSettingsList({
    page,
    pageSize,
    searchParams,
  })

  const createMutation = useCreateJobBaseSetting()
  const updateMutation = useUpdateJobBaseSetting()
  const deleteMutation = useDeleteJobBaseSettings()
  ...
  const handleDelete = useCallback(async () => {
    ...
    try {
      ...
      message.success('岗位基础数值删除成功')
      setSelectedRowKeys([])
    } catch (error) {
      if (error instanceof Error) {
        message.error(error.message)
      } else {
        message.error('删除岗位基础数值失败，请稍后重试')
      }
    }
  }, [deleteMutation, message, selectedRowKeys])

  const handleFinish = useCallback(
    async (values: JobBaseSettingFormValues) => {
      try {
        if (isEdit && selectedRowKeys[0]) {
          ...
          message.success('岗位基础数值更新成功')
        } else {
          await createMutation.mutateAsync(values)
          message.success('岗位基础数值创建成功')
        }
  ...
  const handleSearch = useCallback(
    (params: typeof searchParams) => {
      ...
      if (params.jobName) {
        nextSearchParamsURL.set('jobName', params.jobName)
      } else {
        nextSearchParamsURL.delete('jobName')
      }
  ...
  return (
    <div className="grid h-full grid-rows-[auto_auto_1fr] gap-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <EditButton title="编辑岗位基础数值" handleEdit={handleEdit} />
        <DeleteButton
