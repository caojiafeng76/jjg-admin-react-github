...
export default function AppErrorView({
  ...
  const variantConfig = {
    generic: {
      ...
      headline: '当前操作已被系统安全中断。你可以先刷新页面，或回到首页重新进入模块，避免在半加载状态下继续操作。',
      suggestions: [
    ...
    network: {
      ...
      headline: '系统更像是暂时没有连上数据源，而不是页面本身损坏。优先刷新页面或稍后重试，通常可以自动恢复。',
      suggestions: [
    ...
    permission: {
      ...
      headline: '当前页面已经成功打开，但系统拒绝了这次访问或数据读取。这个问题通常需要检查账号角色、员工绑定或数据库权限策略。',
      suggestions: [
    ...
    notFound: {
      ...
      headline: '这不是系统崩溃，而是当前地址没有命中有效页面。通常是旧链接失效、手动改了地址，或当前账号没有这个入口。',
      suggestions: [
  ...
  return (
    <div className={`relative min-h-screen overflow-hidden px-4 py-10 sm:px-6 lg:px-8 ${variantConfig.shell}`}>
      ...
      <div className={`absolute left-[-96px] top-12 h-56 w-56 rounded-full blur-3xl ${variantConfig.glowLeft}`} />
      <div className={`absolute bottom-0 right-[-72px] h-72 w-72 rounded-full blur-3xl ${variantConfig.glowRight}`} />
      ...
      <div className="relative mx-auto flex min-h-[calc(100vh-5rem)] max-w-4xl items-center justify-center">
        <Card className="w-full overflow-hidden rounded-[32px] border-0 bg-white/82 shadow-[0_32px_90px_rgba(15,23,42,0.16)] backdrop-blur dark:bg-slate-950/75">
          ...
          <div className="grid gap-8 p-2 lg:grid-cols-[1.1fr_0.9fr] lg:gap-10 lg:p-4">
            <div className="rounded-[28px] bg-white/65 p-6 dark:bg-white/5 sm:p-8">
              <Space direction="vertical" size={20} className="w-full">
                <div className="flex items-center gap-3">
                  <div className={`flex h-12 w-12 items-center justify-center rounded-2xl text-[22px] ${variantConfig.iconBox}`}>
                    {variantConfig.icon}
                  </div>
                  <div>
                ...
                <div>
                  <Title className="!mb-3 !text-3xl !leading-tight !text-slate-900 dark:!text-slate-50 sm:!text-4xl">
                    {title}
                  </Title>
                  <Paragraph className="!mb-0 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300 sm:text-[15px]">
                    {variantConfig.headline}
                  </Paragraph>
                </div>

                <div className={`rounded-[24px] border p-5 ${variantConfig.summaryBox}`}>
                  <Text className={`block text-[11px] uppercase tracking-[0.3em] ${variantConfig.summaryTitle}`}>
                    错误摘要
                  </Text>
                  <Paragraph className="!mb-0 !mt-3 text-sm leading-7 text-slate-700 dark:text-slate-200">
                    {description}
                  </Paragraph>
                </div>
            ...
            <div className="flex flex-col justify-between gap-4 rounded-[28px] border border-slate-200/70 bg-slate-950 p-6 text-slate-100 dark:border-white/10 sm:p-8">
              <div>
                <Text className="block text-[11px] uppercase tracking-[0.3em] text-slate-400">
                  处理建议
                </Text>
                <Space direction="vertical" size={14} className="mt-4 flex w-full">
                  ...
                  <div className={`rounded-2xl border border-dashed p-4 ${variantConfig.accentCard}`}>
              ...
              {detail || code ? (
                <div className="rounded-[24px] border border-white/10 bg-white/5 p-5">
                  ...
                  {detail ? (
                    <Paragraph className="!mb-0 !mt-3 break-all font-mono text-xs leading-6 text-slate-300">
                      {detail}
                    </Paragraph>
                  ) : null}
                </div>
                ...
                <div className="rounded-[24px] border border-white/10 bg-white/5 p-5">
                  ...
                  <Paragraph className="!mb-0 !mt-3 text-sm leading-7 text-slate-300">
