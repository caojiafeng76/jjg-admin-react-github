...
7. 如果任务会修改代码、脚本、SQL、配置或指令文件，默认按 Spec Workflow 顺序推进：先判断当前属于 `explore`、`propose`、`apply`、`archive` 哪一阶段；准备写代码时，不要跳过 `propose` / `tasks` 直接进入实现。
8. 先搜索和阅读相关上下文，再决定实施方案；不要在缺少上下文时直接修改。
9. 如果信息不足以继续，只提出最少必要的澄清问题；如果可以合理决策，直接推进。
10. 给出简短执行计划，并明确说明本次准备如何验证，然后开始实施。
11. 如果要修改代码、配置、脚本、SQL 或指令文件，实施前先检查相关文件当前状态；若已有本地改动，必须先阅读并在其基础上修改。
12. 实施时优先修复根因，保持改动最小化，避免顺手修改无关代码。
13. 对有联动风险的改动，必须主动补查：
...
14. 改动后执行必要验证；除纯文档文本调整外，不能在未做任何验证的情况下结束任务。
15. 若无法完成验证，必须明确说明阻塞原因、已尝试替代方案和剩余风险。
16. 如果当前任务对应 change 已完成实现范围，结尾应提示进入 `archive`，不要只停在“代码改完”。
17. 最终输出固定包含以下四项：
