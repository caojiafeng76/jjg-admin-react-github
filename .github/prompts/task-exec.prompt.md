...
2. 所有任务开始前，必须先调用 Sequential Thinking MCP；即使任务很小，也至少要用它明确目标、约束或执行路径。中等及以上复杂度任务，必须进一步用它拆解问题、明确假设、风险和执行顺序，再进入检索和实施。
3. 明确列出准备先查看的代码、配置或文档位置。
4. 建立上下文时必须先调用 Serena MCP 做符号级检索、引用关系和精确定位；若 Serena 当前不可用或返回 `No active project`，仍需明确说明已降级，再退回常规搜索。
...
18. 若无法完成验证，必须明确说明阻塞原因、已尝试替代方案和剩余风险。
19. 如果当前任务对应 change 已完成实现范围，结尾应提示进入 `archive`，不要只停在“代码改完”。
20. 最终输出固定包含以下四项：
