...
8. 给出简短执行计划，并明确说明本次准备如何验证，然后开始实施。
9. 如果要修改代码、配置、脚本、SQL 或指令文件，实施前先检查相关文件当前状态；若已有本地改动，必须先阅读并在其基础上修改。
10. 实施时优先修复根因，保持改动最小化，避免顺手修改无关代码。
11. 对有联动风险的改动，必须主动补查：
...
12. 改动后执行必要验证；除纯文档文本调整外，不能在未做任何验证的情况下结束任务。
13. 若无法完成验证，必须明确说明阻塞原因、已尝试替代方案和剩余风险。
14. 最终输出固定包含以下四项：
