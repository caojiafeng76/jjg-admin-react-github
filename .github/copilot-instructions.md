...
8. 修改代码、配置、脚本、SQL、指令或文档后，必须在根目录 `CHANGELOG.MD` 顶部追加一条记录（日期、类别、范围、摘要、验证），与本次改动在同一工作区变更中完成；禁止把日志留到后续任务
9. 改动后执行必要验证，优先使用与任务匹配的最小验证手段，如 `bun run test`、类型检查、lint、build、局部回归验证；完成任务后必须确保测试通过才能交付，测试未通过时不得声称任务完成
10. 最终回复必须明确说明：

- 完成了什么
- 关键改动是什么
- 是否已更新 `CHANGELOG.MD`（未更新必须说明原因）
- 做了哪些验证
- 是否存在剩余风险、限制或下一步建议
...
- 更新 `CHANGELOG.MD` 是任务完成的硬性条件：每次修改代码、配置、脚本、SQL、指令或文档后，必须在根目录 `CHANGELOG.MD` 顶部追加一条记录（见上文默认流程第 8 步）；仅修改 `CHANGELOG.MD` 以补充日志时无需重复记录。
- 每条记录必须包含日期、类别、范围、修改摘要和验证结果；最新记录置顶，不得改写或删除历史记录。
- 更新日志必须与本次修改在同一个工作区变更中完成，不能把记录留到后续任务。
- 最终汇报必须明确说明是否已更新 `CHANGELOG.MD`；如果任务只涉及讨论或纯只读查询，说明原因即可。

## 严格执行的附加规则

1. 只要任务会修改代码、配置、脚本、SQL 或指令文件，开始编辑前必须先检查相关文件当前状态；如果工作区已有用户改动，必须先阅读并在其基础上修改，禁止盲改覆盖。
2. 只要任务包含实际改动，实施前必须明确本次准备采用的验证方式；除非是纯文档文本调整，否则不能在未做任何验证的情况下结束任务。完成任务后必须确保测试通过才能交付；如果测试未通过，必须继续修复或明确阻塞原因。
3. 如果无法完成验证，必须明确写出阻塞原因、已尝试的替代方案，以及当前结果为什么仍然可信；不能把“未验证”包装成“已完成验证”。
4. 修改 `package.json`、脚本、环境变量、开发流程、MCP 配置或项目指令时，必须同步检查 README、相关 prompt / instructions 是否也需要更新。
